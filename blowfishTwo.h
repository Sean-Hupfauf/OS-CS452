...
#ifndef BLOWFISHTWO_INCLUDED
#define BLOWFISHTWO_INCLUDED
...
class BLOWFISHTWO{
    ...
    #define ROUNDS 16
    public:
        BLOWFISHTWO(std::string hexKey);
        BLOWFISHTWO(byte* cipherKey, int keylength);
    ...
    protected:
        ...
        byte* Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHTWO::*CryptBlock)(byte*, int offset), bool decrypt);
        byte* Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHTWO::*CryptBlock)(byte*, int offset), bool decrypt);
    ...
    BLOWFISHTWO::BLOWFISHTWO(std::string hexKey)
    {
        IvSet = false;
    ...
    int BLOWFISHTWO::hex2dec(char hex)
    {
        if('a' <= hex && hex <= 'f')
            return 10 + (hex - 'a');
        if('A' <= hex && hex <= 'F')
    ...
    BLOWFISHTWO::BLOWFISHTWO(byte* cipherKey, int keyLength)
    {
        IvSet = false;
        SetupKey(cipherKey, keyLength);
    }

    byte* BLOWFISHTWO::Encrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHTWO::Encrypt_Block, false);
    }

    byte* BLOWFISHTWO::Decrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHTWO::Decrypt_Block, true);
    }

    byte* BLOWFISHTWO::Encrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHTWO::Encrypt_Block, false);
    }

    byte* BLOWFISHTWO::Decrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHTWO::Decrypt_Block, true);
    }

    std::string BLOWFISHTWO::Encrypt_CBC(std::string data)
    {
    ...
    std::string BLOWFISHTWO::Decrypt_CBC(std::string data)
    {
        if(data.length() % 2 != 0)
            throw 2;
    ...
    std::string BLOWFISHTWO::byteToHex(unsigned char x)
    {
    ...
    byte* BLOWFISHTWO::padData(byte* data, int length, int* paddedLength, bool decrypt, bool IvSpace = false)
    {
        int offset = 0;
        int dataoffset = 0;
        if(decrypt)
    ...
    int BLOWFISHTWO::findPaddingEnd(byte* data, int length)
    {
        int i = length;
    ...
    byte* BLOWFISHTWO::Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHTWO::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
    ...
    byte* BLOWFISHTWO::Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHTWO::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
        if(!decrypt && !IvSet)
            SetRandomIV();
    ...
    void BLOWFISHTWO::SetIV(byte* newIV)
    {
        IvSet = true;
        for(int i = 0; i < 8; i++)
            IV[i] = newIV[i];
    ...
    void BLOWFISHTWO::Encrypt_Block(byte* block, int offset)
    {
    ...
    void BLOWFISHTWO::Decrypt_Block(byte* block, int offset)
    {
        setblock(block,offset);
        decipher();
        getblock(block,offset);
    ...
    void BLOWFISHTWO::setblock(byte* block, int offset)
    {
    ...
    void BLOWFISHTWO::getblock(byte* block, int offset)
    {
        //TODO: CHECK ENDIANNESS
    ...
    void BLOWFISHTWO::SetupKey(byte* cipherKey, int length)
    {
        if(length > 56)
        {
            throw 56;
    ...
    unsigned int BLOWFISHTWO::round(unsigned int a, unsigned int b, unsigned int n)
    {
        //TODO: CHECK ENDIANNESS
